...
import { Search, LayoutGrid, List, ExternalLink, MoreVertical, Upload, Download } from "lucide-react";
...
export default function FeedDirectory() {
  ...
  const { allFeeds, enabledFeeds, addFeed, updateFeed, removeFeed, toggleFeedEnabled, importFeeds, refresh } = useUserFeeds();
  ...
  const filteredFeeds = useMemo(() => {
    let result = categoryParam ? allFeeds.filter(f => f.category === categoryParam) : [...allFeeds];
    if (qParam) {
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          <div className="relative">
            ...
            {localQ && <button onClick={clearSearch} className="absolute right-2 top-1/2 -translate-y-1/2 text-[0.6875rem] text-slate-500 hover:text-slate-700" type="button">Clear</button>}
          </div>
          ...
          <Button type="button" variant="outline" size="sm" onClick={handleImportClick}><Upload size={16} /> Import</Button>
          <input ref={fileInputRef} type="file" accept=".opml,.xml,text/xml" onChange={handleFileChange} className="hidden" />
          <Button type="button" variant="outline" size="sm" onClick={handleExportOpml}><Download size={16} /> Export</Button>
      ...
      {feedStats && (
        <div className="flex flex-wrap items-center gap-2 text-xs text-slate-600 bg-slate-50 border border-slate-200 rounded-lg px-3 py-2">
          ...
          {feedStats.feedsWithRecentError > 0 && (
            <span className="text-amber-700">{feedStats.feedsWithRecentError} with recent errors</span>
          )}
            ...
            <div key={feed.id} className="card-hover p-4 text-left">
              <div className="flex items-start gap-3">
                ...
                <div className="min-w-0 flex-1">
                  <button onClick={() => navigate(`/feed/${feed.id}`)} className="w-full text-left" type="button">
                  ...
                  <div className="flex items-center gap-2 mt-3">
                    <Switch checked={feed.enabled !== false} onCheckedChange={() => toggleFeedEnabled(feed.id)} aria-label={`${feed.shortName} enabled`} />
                    {isUserFeed(feed) && <FeedFormDialog mode="edit" feed={feed} onSave={updates => updateFeed(feed.id, updates)} trigger={<Button type="button" variant="ghost" size="icon-sm" data-testid="edit-feed"><span className="sr-only">Edit</span>Edit</Button>} />}
                    {isUserFeed(feed) && <Button type="button" variant="ghost" size="sm" onClick={() => removeFeed(feed.id)} data-testid="remove-feed">Remove</Button>}
                  </div>
                </div>
              </div>
            </div>
              ...
              <div className="flex items-center gap-1 flex-shrink-0">
                ...
                <DropdownMenu>
                  <DropdownMenuTrigger asChild>
                    <button type="button" className="p-1.5 rounded-md hover:bg-slate-100 text-slate-400 hover:text-slate-600" aria-label={`${feed.shortName} actions`}><MoreVertical size={14} /></button>
                  </DropdownMenuTrigger>
                  <DropdownMenuContent align="end">
                    {isUserFeed(feed) && (
                      <DropdownMenuItem asChild>
                        <FeedFormDialog mode="edit" feed={feed} onSave={updates => updateFeed(feed.id, updates)} trigger={<span className="w-full cursor-pointer">Edit</span>} />
                      </DropdownMenuItem>
                    )}
                    {isUserFeed(feed) && (
                      <DropdownMenuItem onClick={() => removeFeed(feed.id)} className="text-red-600">Remove</DropdownMenuItem>
                    )}
                    {!isUserFeed(feed) && <DropdownMenuItem disabled>Built-in feed</DropdownMenuItem>}
                  </DropdownMenuContent>
                </DropdownMenu>
              </div>
            </div>
