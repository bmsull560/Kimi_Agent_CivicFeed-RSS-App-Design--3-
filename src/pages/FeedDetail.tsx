...
export default function FeedDetail() {
  ...
  const { status, entries, error, refresh } = useRssFeed(feed?.rssUrl || "", feed?.id || "");
  ...
  if (!feed)
    return (
      <EmptyState
        ...
        subMessage={`No feed exists with ID "${id}".`}
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-start gap-3">
        ...
        <div className="min-w-0 flex-1">
          <div className="flex items-center gap-2 flex-wrap">
            ...
            <span
              className={`w-2 h-2 rounded-full ${statusDot(feed.status)}`}
              title={statusLabel(feed.status)}
            />
          </div>
          ...
          <div className="flex flex-wrap items-center gap-2 mt-2">
            ...
            {feed.subCategory !== feed.category && (
              <span className="badge bg-slate-50 text-slate-500">{feed.subCategory}</span>
            )}
          </div>
        </div>
      </div>
      ...
      <div className="flex flex-wrap items-center gap-2">
        ...
        <button onClick={handleCopyUrl} className="btn-secondary" type="button">
          <Copy size={16} /> Copy RSS URL
        </button>
        {feed.website && (
          <a
            href={feed.website}
        ...
        <a href={feed.rssUrl} target="_blank" rel="noopener noreferrer" className="btn-secondary">
          <Rss size={16} /> Raw Feed
        </a>
      </div>
      ...
      {status === "loading" && entries.length === 0 ? (
        <LoadingState count={5} type="list-item" />
        ...
        <div className="card">
          <EmptyState
            ...
            subMessage={
              error ||
              "The feed could not be fetched. This may be due to CORS restrictions or the feed being unavailable."
            }
        ...
        <div className="card">
          <EmptyState
            ...
            subMessage="This feed may be empty or temporarily unavailable."
            ...
            <div>
              ...
              {status === "error" && error && (
                <p className="text-xs text-amber-700 mt-1">
                  Showing cached entries. Refresh failed: {error}
                </p>
              )}
            </div>
            {status === "loading" && (
