...
import { Shield, Search, Rss, Menu, X, Moon, Sun } from "lucide-react";
...
export default function Header({ onMenuToggle, sidebarOpen }: HeaderProps) {
  ...
  const enabledFeedCount = useMemo(
    () => allFeeds.filter((f) => f.enabled !== false).length,
    [allFeeds]
  );
      ...
      <div className="px-4 lg:px-6 pb-3">
        <div className="relative max-w-xl" ref={dropdownRef}>
          ...
          <input
            ...
            placeholder="Search feeds... (press / to focus)"
            className="input w-full pl-9 pr-4 py-2 text-sm bg-background"
          ...
          {isDropdownVisible && results.length === 0 && (
            <div className="absolute top-full left-0 right-0 mt-1 bg-white border border-slate-200 rounded-lg shadow-lg z-50 px-4 py-3 text-sm text-slate-500">
