...
export function useRssFeed(feedUrl: string, feedId: string, _feedName: string): FetchState & { refresh: () => void } {
  ...
  const doFetch = useCallback(async (force: boolean) => {
    ...
    setState((s) => ({ ...s, status: s.entries.length > 0 ? "loading" : "loading", error: null }));
