...
Standardize how U.S. government RSS/Atom/XML feeds are researched, classified, fetched, normalized, and inserted into CivicFeed's static feed database.

## When to Use

Use this skill when you:

- Add one or more new feeds to `src/data/feeds.ts`
- Research or discover candidate government feeds
- Update an existing feed URL, title, category, or status
- Validate that a source URL is parseable by the app's pipeline
...
3. **Fetch Compatibility Check** — Run the same path the app uses:
   - Direct fetch with `User-Agent: Feedly/1.0 (+http://www.feedly.com/fetcher.html)` and `Accept: application/rss+xml, application/atom+xml, application/xml, text/xml, */*` (12s timeout).
   - If direct fails, try each proxy fallback with 18s timeout.
   - Confirm `parseRssXml` returns at least one entry (smoke test).
...
5. **Schema Adaptations**
   - **Atom dates**: prefer `<published>`, fallback to `<updated>`; normalized via `normalizeDate`.
   ...
   - **HTML fallback**: when XML parsing emits `parsererror` but the raw response contains `<item>` tags, items are parsed from the HTML document; links may be relative.
...
7. **Data Insertion**
   - Append the new `Feed` object to the `feeds` array in `src/data/feeds.ts`.
...
| HTML-embedded RSS | `parsererror` in XML, but `<item>` tags exist in raw HTML | Fallback | Parse items from HTML; watch for relative links. |
...
2. **Forgetting to update `feedStats`** — Fix: increment `total`, `byCategory[category]`, and `byStatus[status]`.
3. **Using uppercase tags or free-form casing** — Fix: convert every tag to lowercase.
4. **Duplicate `id` values** — Fix: assign the next sequential `feed-NNN` and verify uniqueness.
5. **Leaving `status` as `"unverified"` after a successful fetch+parse** — Fix: set `"working"` when the smoke test returns entries.
6. **Assigning the wrong priority tier** — Fix: match the agency mission to the tier table.
7. **Accepting relative `rssUrl` or unresolved relative item links** — Fix: store absolute HTTPS URLs; resolve relative links against `website` or the feed's base URL.

## Quick Reference

Before saving changes to `src/data/feeds.ts`, confirm:

- [ ] URL fetched live and returned parseable entries (direct or proxy)
